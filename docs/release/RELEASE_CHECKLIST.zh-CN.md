# CommerceFlow Agent v1.1.0 发布清单

本清单用于维护者发布 GitHub Release。`v1.1.0` tag 只有在后端、前端 E2E、镜像和 Windows 启动器构建全部成功后才会自动发布为 Latest；`v1.0.0` 保留为可回退版本。
...
```powershell
git tag -a v1.1.0 -m "CommerceFlow Agent v1.1.0"
...
- 在未登录 GitHub 的环境中匿名拉取 `v1.1.0` 镜像。
